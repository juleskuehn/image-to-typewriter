  ...
  drawNumbers = ->
    ...
    drawStreak = (start,streakLength,color) ->
      ...
      if streakLength>3
        ...
        ctx.fillText(streakLength,colStart+charset.qWidth*0.5,rowStart+charset.qHeight*1.5)
...
setupMicrophone = ->
  ...
  window.analyzer = undefined

  window.setup = ->
    # Create an Audio input
    window.input = new (p5.AudioIn)
    window.input.start()
    return

  window.draw = ->
    # Get the overall volume (between 0 and 1.0)
    window.volume = window.input.getLevel()
    # If the volume > 0.1,  a rect is drawn at a random location.
    # The louder the volume, the larger the rectangle.
    threshold = 0.2
    if window.volume > threshold
      if tabState == 'layers' and typing[typing.selectedLayer].col < combosArray[0].length / 2
        typing[typing.selectedLayer].col++
        drawTypingTools typing.selectedLayer
    return
